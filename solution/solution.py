...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    header = f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | {'GPT-4o Latency':>14} | {'Mini Latency':>13}"
    separator = "-" * len(header)
    lines = [header, separator]
    for r in results:
        prompt = r["prompt"][:40]
        gpt4o_resp = r["gpt4o_response"][:40]
        mini_resp = r["mini_response"][:40]
        gpt4o_lat = r["gpt4o_latency"]
        mini_lat = r["mini_latency"]
        lines.append(
            f"{prompt:<40} | {gpt4o_resp:<40} | {mini_resp:<40} | {gpt4o_lat:>14.3f} | {mini_lat:>13.3f}"
        )
    return "\n".join(lines)
